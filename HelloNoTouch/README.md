...
### Build
1. Open "HelloNoTouch\BuildMFC\HelloNoTouch.sln" by Visual studio 2017
2. Click `F5` to build/run `HelloNoTouch`
...
### Build
1. Open "HelloNoTouch\BuildSTM32F103-Keil\USER\HelloNoTouch.uvprojx" with Keil uvsion.
2. Choose your Device type(Default: STM32F103ZE) from option for target.
3. Build HelloNoTouch.
...
4. Link your LCD driver with GuiLite, and call UI entry function(e.g, startHelloNoTouch)
...
- `sudo ./HelloNoTouch /dev/fb0`&nbsp;&nbsp;&nbsp;&nbsp;/dev/fb0: The path of framebuffer device file.

3. Run inside QT APP(display-xxx is a QT APP for display, skip this if you haven't installed QT):
- If x64: `sudo ./display-x64 1 240 320 | ./HelloNoTouch shared-fb`
- If ARM/Raspberry Pi: `sudo ./display-arm  1 240 320 | ./HelloNoTouch shared-fb`
...
2. How to run HelloNoTouch on ARM Linux device?
- Copy HelloNoTouch you build on ARM Linux device
- `sudo ./HelloNoTouch /dev/fb0`&nbsp;&nbsp;&nbsp;&nbsp;/dev/fb0: The path of framebuffer
