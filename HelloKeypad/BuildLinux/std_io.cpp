...
extern int captureUiOfHelloKeypad();
...
static void* stdin_thread(void* param)
{
	...
	while (true)
	{
		...
		{
			captureUiOfHelloKeypad();
			printf("snapshot done.");
		}
		else if (strcmp(buffer, "d") == 0)
		{
			sendKey2HelloKeypad(0);
			printf("forward pressed.");
		}
		else if (strcmp(buffer, "a") == 0)
		{
			sendKey2HelloKeypad(1);
			printf("backward pressed.");
		}
		else if (strcmp(buffer, "s") == 0)
		{
			sendKey2HelloKeypad(2);
		...
		else if (strstr(buffer, "press") == buffer)
		{
			...
			sendTouch2HelloKeypad(x, y, true);
			printf(buffer);
		}
		else if (strstr(buffer, "release") == buffer)
		{
			int x, y;
			sscanf(buffer, "%*[a-z|(]%d,%d", &x, &y);
			sendTouch2HelloKeypad(x, y, false);
			printf(buffer);
		}
		else
		{
