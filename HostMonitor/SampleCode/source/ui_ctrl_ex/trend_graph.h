...
public:
	...
	void set_h_axis_atrrs(const GUI_FONT* font, long scale_value[], int scale_count);//time in seconds.
	void set_v_axis_atrrs(unsigned int v_axis_index, unsigned int color, const GUI_FONT* font, int scale_value[], int scale_count);
...
private:
	...
	const GUI_FONT* m_h_axis_mark_font;
	...
	const GUI_FONT* m_v_axis_mark_font[V_AXIS_CNT];
