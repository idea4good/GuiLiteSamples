...
static c_slide_group 	s_page_group;
//right values region
...
static WND_TREE s_root_children[] =
{
	...
	//left
	{ (c_wnd*)&s_page_group, ID_PAGE_GROUP, 0, 0, TOP_BAR_HEIGHT, (SCREEN_WIDTH * 2 / 3), (SCREEN_HEIGHT - TOP_BAR_HEIGHT)},
	//right values
...
void load_ui_single(void* phy_fb)
{
	...
	create_page_ecg_7wave(&s_page_group);
	create_page_main(&s_page_group);
	create_page_trend(&s_page_group);
	create_page_config(&s_page_group);
	s_page_group.set_active_slide(1);
	s_root.show_window();

	new c_gesture(&s_root, &s_page_group, display->get_hid_pipe());
}
...
static c_slide_group 		s_mini_page_group;
static c_mini_value_view	s_value_view;

static WND_TREE s_mini_root_children[] =
{
	{ (c_wnd*)&s_mini_page_group, ID_PAGE_GROUP, 0, 0, 0, (MINI_SCREEN_WIDTH * 2 / 3), MINI_SCREEN_HEIGHT },
...
void load_mini_ui_single(void* phy_fb)
{
	...
	create_page_mini_wav(&s_mini_page_group);
	create_page_mini_trend(&s_mini_page_group);
	s_mini_page_group.set_active_slide(0);
	s_mini_root.show_window();

	new c_gesture(&s_mini_root, &s_mini_page_group, display->get_hid_pipe());
}
