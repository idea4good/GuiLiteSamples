...
extern void* getUiOfHelloScroll(int* width, int* height, bool force_update);
...
class GuiLiteThread: public QThread
{
    void run()
    {
        startHelloScroll(calloc(UI_WIDTH * UI_HEIGHT, 2), UI_WIDTH, UI_HEIGHT, 2);
    }
};
...
void MainWindow::paintEvent(QPaintEvent* p)
{
    ...
    void* fb = getUiOfHelloScroll(NULL, NULL, force_update);
    if(fb){
...
{
    m_is_pressed = true;
    sendTouch2HelloScroll(e->pos().x(), e->pos().y(), m_is_pressed);
}
