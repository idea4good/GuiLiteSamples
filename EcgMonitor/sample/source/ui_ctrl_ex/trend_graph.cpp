...
void c_trend_graph::draw_h_axis(void)
{
	...
	p->set_font(m_h_axis_mark_font);
	p->set_color(H_AXIS_COLOR, BACKGROUND_COLOR);
	...
	for ( int i = 0; i < m_h_scale_cnt; i++ )
	{
		...
		//Draw mark
		p->draw_string(str_scale_value, x_start - 20, y_start + 5 );
	}
}
...
void c_trend_graph::draw_v_axis(int index)
{
	...
	c_word *p = c_word::get_instance(m_z_order, m_surface);
	p->set_font(m_v_axis_mark_font[index]);
	p->set_color(m_v_axis_color[index], BACKGROUND_COLOR);
	...
	for ( int i = 0; i < m_v_scale_cnt[index]; i++ )
	{
		...
		p->draw_string( str_scale_value, mark_text_x, (scale_line_y - 8));//mark
	}
}
...
void c_trend_graph::draw_title(unsigned int row_index, unsigned int str_id, unsigned int color, const GUI_FONT* font)
{
	...
	c_word *p = c_word::get_instance(m_z_order, m_surface);
	p->set_font(font);
	p->set_color(color, BACKGROUND_COLOR);
	p->draw_string(str_id, rect.m_left + MARGIN_OF_TITLE, (rect.m_top + TITLE_HEIGHT * row_index + 5));
}
