// HelloScroll.cpp : Defines the entry point for the application.
//

#include "stdafx.h"
#include "UiBlock.h"
#include "HelloScroll.h"
#include <windowsx.h>
#include <shellapi.h>
...
DWORD WINAPI ThreadHelloScroll(LPVOID pParam);
                     ...
                     _In_ int       nCmdShow)
{
    ...
    LoadStringW(hInstance, IDC_HelloScroll, szWindowClass, MAX_LOADSTRING);
    ...
    HACCEL hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_HelloScroll));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
    ...
    wcex.hIcon          = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_HelloScroll));
    wcex.hCursor        = LoadCursor(nullptr, IDC_ARROW);
    wcex.hbrBackground  = (HBRUSH)(COLOR_WINDOW+1);
    wcex.lpszMenuName   = MAKEINTRESOURCEW(IDC_HelloScroll);
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
   ...
   DWORD threadID;
   CreateThread(NULL, 0, ThreadHelloScroll, NULL, 0, &threadID);
...
//////////////////////// HelloScroll Interface ////////////////////////
void startHelloScroll(void* phy_fb, int width, int height, int color_bytes);

DWORD WINAPI ThreadHelloScroll(LPVOID pParam)
{
	startHelloScroll(calloc(600 * 400, COLOR_BYTES), 600, 400, COLOR_BYTES);
	return 0;
}
