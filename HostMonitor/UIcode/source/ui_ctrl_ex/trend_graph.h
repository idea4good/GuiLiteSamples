...
public:
	...
	void set_h_axis_atrrs(const LATTICE_FONT_INFO* font, long scale_value[], int scale_count);//time in seconds.
	void set_v_axis_atrrs(unsigned int v_axis_index, unsigned int color, const LATTICE_FONT_INFO* font, int scale_value[], int scale_count);
...
private:
    ...
    void draw_title(unsigned int row_index, char* str, unsigned int color, const LATTICE_FONT_INFO* font);
...
private:
	...
	const LATTICE_FONT_INFO* m_h_axis_mark_font;
	...
	const LATTICE_FONT_INFO* m_v_axis_mark_font[V_AXIS_CNT];
